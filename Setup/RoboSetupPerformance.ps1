...
function Main {
    ...
    if ($shouldConnect) {
		if ($Tennant) {
			...
			$perfConnect = ConnectTo-Orchestrator-Perf -orchestratorUrl $orchestratorUrl -robotExePath $robotExePath -tennant $tennant -adminUsername $adminUsername -adminPassword $adminPassword -HostingType $HostingType -RobotType $RobotType
...
function ConnectTo-Orchestrator-Perf {
   ...
   $dataRobot = @{
    ...
    Write-Host $key
    Start-Process $robotExePath -ArgumentList "--connect -url  $orchestratorUrl -key $key"
	}
Main
